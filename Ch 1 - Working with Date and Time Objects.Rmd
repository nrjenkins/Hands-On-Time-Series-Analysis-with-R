...
1.  `Date`: this basic format puts dates in the `YYY-m-d` format and tracks the number of days since the default of `1970-01-01`.
2.  `POSIXct`/`POSIXlt`: Known as a `DateTime` class and they represent the calendar date, the time of daay, and the time zone using the ISO 8601 international standard format of `YYY-m-d H:M:S`. `POSIXct` represents the number of seconds since the origin point as a numeric vector and `POSIXlt` stores each one of the date and time elements as a list.
